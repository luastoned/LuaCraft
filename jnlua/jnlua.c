...
static void pushuserdata(lua_State *L, jobject object);
...
static int openlib_protected (lua_State *L) {
	...
	case 9:
		...
		libname = LUA_JITLIBNAME;
		break;
	case 10:
		openfunc = luaopen_ffi;
		libname = LUA_FFILIBNAME;
		break;
	default:
		return 0;
	} 
...
/* lua_pushuserdata() */
JNLUA_THREADLOCAL jobject pushuserdata_object;
static int pushuserdata_protected (lua_State *L) {
	pushuserdata(L, pushuserdata_object);
	return 1;
}
JNIEXPORT void JNICALL Java_com_naef_jnlua_LuaState_lua_1pushuserdata (JNIEnv *env, jobject obj, jobject object) {
	lua_State *L;
	...
	if (checkstack(L, JNLUA_MINSTACK)
			&& checknotnull(object)) {
		pushuserdata_object = object;
		lua_pushcfunction(L, pushuserdata_protected);
		JNLUA_PCALL(L, 0, 1);
	}
}
...
/* lua_isuserdata() */
JNIEXPORT jint JNICALL Java_com_naef_jnlua_LuaState_lua_1isuserdata (JNIEnv *env, jobject obj, jint index) {
	lua_State *L;
	...
	return (jint) lua_isuserdata(L, index);
}
...
/* lua_touserdata() */
JNLUA_THREADLOCAL jobject touserdata_result;
static int touserdata_protected (lua_State *L) {
	touserdata_result = touserdata(L, 1);
	return 0;
}
JNIEXPORT jobject JNICALL Java_com_naef_jnlua_LuaState_lua_1touserdata (JNIEnv *env, jobject obj, jint index) {
	lua_State *L;
	...
	if (checkstack(L, JNLUA_MINSTACK)
			&& checkindex(L, index)) {
		index = lua_absindex(L, index);
		lua_pushcfunction(L, touserdata_protected);
		lua_pushvalue(L, index);
		JNLUA_PCALL(L, 1, 0);
	}
	return touserdata_result;
}
