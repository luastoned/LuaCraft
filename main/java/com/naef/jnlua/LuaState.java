...
public class LuaState {
	...
	public boolean isUserdata(int index, Class<?> formalType) {
		check();
		...
		case USERDATA:
			if (isJavaObjectRaw(index)) {
				...
				if (object instanceof LuaUserdata) {
					return ((LuaUserdata) object).getTypeName();
				}
			}
			break;
		default:
	...
	public void newMetatable(String meta) {
		check();
	 ...
	 */
	public boolean setMetatable(int index) {
		check();
	...
	public JavaFunction checkJavaFunction(int index) {
		check();
	...
	public Object checkUserdata(int index) {
		check();
		if (!isUserdata(index)) {
	...
	public <T> Object checkUserdata(int index, Class<T> formalType, String expected) {
		check();
	...
	private LuaRuntimeException getArgTypeException(int index, LuaType type) {
		return getArgException(index,
				String.format("expected %s, got %s", type.toString().toLowerCase(), typeName(index)));
	}
