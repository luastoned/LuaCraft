...
public class LuaEventManager
{
	 ...
	 * Calls whenever a command is ran
	 ...
	 * Calls whenever a key is pressed
	...
	// Player Events (ItemCraftedEvent, ItemPickupEvent, ItemSmeltedEvent, PlayerChangedDimensionEvent, PlayerLoggedInEvent, PlayerLoggedOutEvent, PlayerRespawnEvent)
	...
	// Tick Events (ClientTickEvent, PlayerTickEvent, RenderTickEvent, ServerTickEvent, WorldTickEvent)
	...
	{
		// Child class of LivingDropsEvent that is fired specifically when a player dies. Canceling the event will prevent ALL drops from entering the world.
		synchronized (l)
		{
			if (!l.isOpen())
				return;
	...
	// setResult ALLOW to allow the container to stay open setResult DENY to force close the container (denying access)
